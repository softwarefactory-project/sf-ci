...
function terminate {
    ...
    popd
    sudo chown -R ${SUDO_USER}:${SUDO_USER} ${ARTIFACTS}
    [ "$1" == "END" ] && exit 0 || exit 1
}
...
trap 'terminate' ERR
ansible-playbook -e "workspace=${WORKSPACE} sf_ci=$(pwd) sf_arch=${ARCH} sf_version=${VERSION} func_test_case=${FUNC_TEST_CASE}" playbooks/${TEST_TYPE}.yml
