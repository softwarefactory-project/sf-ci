...
class TestUserdata(Base):
    ...
    def test_delete_in_backend_and_recreate(self):
        ...
        auth_cookie = get_cookie("admin", config.USERS[config.ADMIN_USER]['password'])
        d = requests.delete(del_url,
                            cookies={'auth_pubtkt': auth_cookie})
        self.assertTrue(int(d.status_code) < 400, d.status_code)
        subprocess.Popen([
            "sudo", "env", "EMAIL=mrbadguy@queen.com", "/usr/share/sf-config/scripts/delete-gerrit-user.sh",
            "freddie", "--batch"]).wait()
        time.sleep(1)
