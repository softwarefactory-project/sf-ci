    ...
    def wait_for_config_update(self, revision):
        if os.environ.get("SF_JENKINS_EXECUTOR", "0") == "1":
            ...
            for retry in xrange(120):
                ...
                time.sleep(1)
            return job_log
        # Remove legacy code when 2.6 is released
        ...
        try:
            for retry in xrange(120):
                ...
                for result in r.json()['results']:
                    # make sure build is finished
                    if result['end_time']:
                        job_log = result["log_url"]
                        if job_log.startswith('http'):
                            return requests.get("%s/console.html" % job_log).text
                        elif job_log.startswith('telnet://'):
                            host, port = job_log[len('telnet://'):].split(':')
                            logger.debug("Telnet into %s:%i" % (host, int(port)))
                            tn = telnetlib.Telnet(host, int(port))
                            return tn.read_all()
                        else:
                            logger.exception("Unknown protocol %s" % job_log)
                time.sleep(1)
        except:
